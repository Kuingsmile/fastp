...
void FilterResult::addAdapterTrimmed(string adapter, bool isR2,
  ...
  if (!isR2) {
    if (mAdapter1.count(adapter) > 0)
      ...
      if (mAdapter1.size() > MAX_ADAPTER_REC ||
          (mAdapter1.size() > LOW_COMPLEXITY_SKIP && isLowComplexity(adapter)))
        return;
      mAdapter1[adapter] = 1;
    }
  } else {
    if (mAdapter2.count(adapter) > 0)
      ...
      // we skip this adapter when there are too many adapters recorded
      // or when this adapter is with low complexity
      if (mAdapter2.size() > MAX_ADAPTER_REC ||
          (mAdapter2.size() > LOW_COMPLEXITY_SKIP && isLowComplexity(adapter)))
        return;
      mAdapter2[adapter] = 1;
    }
  }
...
void FilterResult::addAdapterTrimmed(string adapter1, string adapter2) {
  ...
  if (!adapter1.empty()) {
    if (mAdapter1.count(adapter1) > 0)
      ...
      // or when this adapter is with low complexity
      if (mAdapter1.size() > MAX_ADAPTER_REC ||
          (mAdapter1.size() > LOW_COMPLEXITY_SKIP && isLowComplexity(adapter1)))
        return;
  ...
  if (!adapter2.empty()) {
    if (mAdapter2.count(adapter2) > 0)
      ...
      // or when this adapter is with low complexity
      if (mAdapter2.size() > MAX_ADAPTER_REC ||
          (mAdapter2.size() > LOW_COMPLEXITY_SKIP && isLowComplexity(adapter2)))
        return;
...
void FilterResult::reportAdapterHtml(ofstream &ofs, long totalBases) {
  if (!mOptions->isPaired()) {
    ofs << "<div class='subsection_title' "
           "onclick=showOrHide('read_adapters')>Adapter or bad "
           "ligation</div>\n";
    ...
    int limitCount =
        min(getAdapterReportCount(mAdapter1), getAdapterReportCount(mAdapter2));
    ofs << "<table> <tr> <td>\n";
    ofs << "<div class='subsection_title' "
...
int FilterResult::getAdapterReportCount(
...
int FilterResult::outputAdaptersHtml(
    ofstream &ofs, map<string, long, classcomp> &adapterCounts, long totalBases,
    int limitCount) {
