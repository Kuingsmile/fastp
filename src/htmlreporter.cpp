...
void HtmlReporter::printSummary(ofstream &ofs, FilterResult *result,
  ...
  if (mOptions->adapterCuttingEnabled()) {
    ...
    if (!mOptions->adapter.detectedAdapter1.empty()) {
      ...
      if (knownAdapters.count(adapterinfo1) > 0)
        adapterinfo1 +=
            " -" + knownAdapters[mOptions->adapter.detectedAdapter1];
    ...
    if (!mOptions->adapter.detectedAdapter2.empty()) {
      ...
      if (knownAdapters.count(adapterinfo2) > 0)
        adapterinfo2 +=
            " -" + knownAdapters[mOptions->adapter.detectedAdapter2];
  ...
  outputRow(ofs, "Q40 bases:",
            formatNumber(pre_q40_bases) + " (" +
                getPercents(pre_q40_bases, pre_total_bases) + "%)");
  outputRow(ofs,
  ...
  outputRow(ofs, "Q40 bases:",
            formatNumber(post_q40_bases) + " (" +
                getPercents(post_q40_bases, post_total_bases) + "%)");
  outputRow(ofs,
...
void HtmlReporter::reportQualHistogram(ofstream &ofs, string caption,
  ...
  ofs << "<div class='figure' style='height:400' id='plot_" + divName +
             "'></div>\n";
  ...
  ofs << "title: '" << caption << "',";
  ofs << "xaxis:{title:'base quality score'}, yaxis:{title:'base count'}}; "
      << endl;
...
void HtmlReporter::report(FilterResult *result, Stats *preStats1,
  ...
  ofs << "<div class='section_div'>\n";
  // ofs << "<div class='section_title'
  // onclick=showOrHide('before_filtering')><a name='summary'>Before
  // filtering</a></div>\n"; ofs << "<div id='before_filtering'>\n";

  ofs << "<div class='section_title' onclick=showOrHide('quality_stat')><a "
         "name='summary'>Filtering statistics</a>";
  if (mOptions->out1.empty() && !mOptions->outputToSTDOUT) {
    // no output, we give a hint here for clarification
    ofs << "<font size=-2> (although no output file specified, the filtered "
           "statistics are still presented here) </font>"
        << endl;
  }
  ofs << "</div>\n";
  ...
  ofs << "<tr><td>\n";
  reportQualHistogram(ofs, "Before filtering: quality score histogram",
                      preStats1, preStats2);
  ofs << "</td><td>\n";
  reportQualHistogram(ofs, "After filtering: quality score histogram",
                      postStats1, postStats2);
  ofs << "</td></tr>\n";
...
void HtmlReporter::printCSS(ofstream &ofs) {
  ...
  ofs << "table {border:1px solid "
         "#999999;padding:2x;border-collapse:collapse;width:100%}"
      << endl;
...
void HtmlReporter::printJS(ofstream &ofs) {
  ofs << "<script src='https://opengene.org/plotly-1.2.0.min.js'></script>"
      << endl;
  ofs << "\n<script type='text/javascript'>" << endl;
