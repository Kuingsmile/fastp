...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2) {
    if (std::string_view arg{argv[1]}; arg == "test") {
      ...
      std::cout << "fastp " << FASTP_VER << std::endl;
      return 0;
    }
  }
  ...
  cmd.add<string>("adapter_fasta", 0,
                  ...
                  false, "");
  cmd.add("detect_adapter_for_pe", '2',
          "enable adapter detection for PE data to get ultra-clean data. It "
          "takes more time to find just a little bit more adapters.");
  cmd.add("allow_gap_overlap_trimming", 0,
          "allow up to one gap when trim adapters by overlap analysis for PE "
          "data. By default no gap is allowed.");
  ...
  cmd.add<string>(
      "umi_delim", 0,
      "delimiter to use between the read name and the UMI, default is :", false,
      ":");
  ...
  if (argc == 1) {
    ...
    cerr << "Citation:" << endl;
    cerr << "Shifu Chen. 2023. Ultrafast one-pass FASTQ data preprocessing, "
            "quality control, and deduplication using fastp. iMeta 2: e107"
         << endl;
    cerr << endl;
    return 0;
  }
  ...
  if (!opt.qualityCut.enabledFront && !opt.qualityCut.enabledTail &&
    ...
    if (cmd.exist("cut_window_size") || cmd.exist("cut_mean_quality") ||
      ...
      cerr << "WARNING: you specified the options for cutting by quality, but "
              "forgot to enable any of cut_front/cut_tail/cut_right. This will "
              "have no effect."
           << endl;
  }
  ...
  if (opt.umi.enabled) {
    ...
    if (umiLoc == "index1") {
      opt.umi.location = static_cast<int>(UMILocation::Index1);
    } else if (umiLoc == "index2") {
      opt.umi.location = static_cast<int>(UMILocation::Index2);
    } else if (umiLoc == "read1") {
      opt.umi.location = static_cast<int>(UMILocation::Read1);
    } else if (umiLoc == "read2") {
      opt.umi.location = static_cast<int>(UMILocation::Read2);
    } else if (umiLoc == "per_index") {
      opt.umi.location = static_cast<int>(UMILocation::PerIndex);
    } else if (umiLoc == "per_read") {
      opt.umi.location = static_cast<int>(UMILocation::PerRead);
    }
  }
