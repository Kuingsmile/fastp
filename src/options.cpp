...
bool Options::isPaired() const { return in2.length() > 0 || interleavedInput; }

bool Options::adapterCuttingEnabled() const {
  if (adapter.enabled) {
...
bool Options::polyXTrimmingEnabled() const { return polyXTrim.enabled; }
...
bool Options::validate() {
  ...
  if (!out1.empty()) {
    ...
    if (out1 == out2) {
      error_exit("read1 output (--out1) and read2 output (--out2) should be "
                 "different");
    }
  ...
  if (umi.enabled) {
    if (umi.location == static_cast<int>(UMILocation::Read1) ||
        umi.location == static_cast<int>(UMILocation::Read2) ||
        umi.location == static_cast<int>(UMILocation::PerRead)) {
...
bool Options::shallDetectAdapter(bool isR2) const {
  if (!adapter.enabled)
    return false;
...
void Options::initIndexFiltering(const std::string &blacklistFile1,
                                 const std::string &blacklistFile2,
                                 int threshold) {
...
Options::makeListFromFileByLine(const std::string &filename) {
  std::vector<std::string> ret;
  std::ifstream file;
  file.open(filename.c_str(), std::ifstream::in);
  const int maxLine = 1000;
  char line[maxLine];
  std::cerr << "filter by index, loading " << filename << std::endl;
...
std::string Options::getAdapter1() const {
...
std::string Options::getAdapter2() const {
